...
public class AdvertController {
	...
	public Object doAdd(HttpServletRequest request, Advert advert ,HttpSession session) {
		...
		try {
			...
			MultipartFile mfile = mreq.getFile("advpic");		//根据名称获取上传的文件
			
			String name = mfile.getOriginalFilename();//java.jpg
			String extname = name.substring(name.lastIndexOf(".")); // .jpg		//获取上传文件的后缀即文件类型
			
			String iconpath = UUID.randomUUID().toString()+extname; //232243343.jpg	随机生成文件名，防止重复
			
			ServletContext servletContext = session.getServletContext();
			String realpath = servletContext.getRealPath("/pic");		//保存的真实路径，相对于webapp目录
			
			String path =realpath+ "\\adv\\"+iconpath;			//完整的文件路径
			
			mfile.transferTo(new File(path));					//把文件写到本地中
